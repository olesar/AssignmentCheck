...
When you get the result metrix from `test_files()` function you can use `mistaken_tasks()` function for generating the new rmarkdown file with fragments of students' works.
...
As you see, It is a new rmarkdown file, where all fragments with unexpected values are named after a student that produce that part of rmarkdown. If you use `write_file = TRUE` it creates a file `"mistaken_tasks.Rmd"` in your working directory. You can open it using `file.edit("mistaken_tasks.Rmd")` command.
