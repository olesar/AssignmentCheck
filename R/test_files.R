...
test_files <- function(expectations, files = list.files(), keyword_prefix = "### ") {
  if(length(files) == 0){
  ...
  results <- test_one_file(file = files[1], expectations = expectations)

  sapply(files[-1], function(x) {
    results <<- rbind.data.frame(results,
                                 test_one_file(file = x, expectations = expectations))
  })
